...
const starts = async (haruka = new WAConnection()) => {
    ...
    haruka.browserDescription = [ 'Rifai Chump', 'Chrome', '3.0' ]
	CFonts.say('Haruka', {
		font: 'block',
    	color: ['#ff9c00'],
    	align: 'center',
	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							...
							try{
							await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Ingin topup dah joki? klik shop`,welcome, [{"buttonId": `.daftar`,"buttonText": {"displayText": "Daftar"},"type": "RESPONSE"},{"buttonId": `.shop`,"buttonText": {"displayText": "Shop"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Ingin topup dan joki? klik shop`,ppmem2, [{"buttonId": `.daftar`,"buttonText": {"displayText": "Daftar"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
